...
| Glider API | Struct-based `glider.v1`; typed `glider.v2` is served as Candidate and used by node secret delivery | Immutable-baseline breaking check, reproducible generated-client compilation, and mTLS generation-fencing tests; CLI migration and mixed-version qualification required before v2 becomes Current |
