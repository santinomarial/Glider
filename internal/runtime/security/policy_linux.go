...
func ApplyDefault() error {
	if err := reduceCapabilities(DefaultCapabilities); err != nil {
		return fmt.Errorf("reduce capabilities: %w", err)
	}
...
func reduceCapabilities(allowed []int) error {
	last := 63
	if raw, err := os.ReadFile("/proc/sys/kernel/cap_last_cap"); err == nil {
		if n, parseErr := strconv.Atoi(strings.TrimSpace(string(raw))); parseErr == nil {
			last = n
		}
	}
	...
	for cap := 0; cap <= last; cap++ {
		if !keep[cap] {
			if err := unix.Prctl(unix.PR_CAPBSET_DROP, uintptr(cap), 0, 0, 0); err != nil && !errors.Is(err, syscall.EINVAL) {
	...
	if err := unix.Prctl(prCapAmbient, prCapAmbientClearAll, 0, 0, 0); err != nil && !errors.Is(err, syscall.EINVAL) {
...
func installDefaultSeccomp() error {
	...
	for _, nr := range blocked {
		filter = append(filter, unix.SockFilter{Code: bpfJMPJEQK, Jf: 1, K: uint32(nr)}, unix.SockFilter{Code: bpfRETK, K: seccompRetErrno | uint32(syscall.EPERM)})
	}
