...
mkdir -p "${EVIDENCE}"
docker run --rm -v "${WORK}:/work" golang:1.26 sh -c \
...
for file in multi-host-lb.log off-host-restore.log rolling-upgrade.log node-replacement.log disk-pressure.log network-qualification.log monitoring-delivery.log security-review.md; do
	printf 'qualification passed\n' > "${EVIDENCE}/${file}"
done
docker run --rm -v "${WORK}:/work" -w /work/evidence golang:1.26 sh -c \
