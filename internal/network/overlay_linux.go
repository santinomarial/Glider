...
type Peer struct {
	...
	PodCIDR       netip.Prefix `json:"pod_cidr"`
...
func (m *Manager) EnsureOverlay(localTunnel netip.Addr, peers []Peer, mtu int) error {
	if !localTunnel.Is4() {
		return errors.New("local tunnel address must be IPv4")
	}
	if mtu == 0 {
		mtu = 1450
	}
	...
	_ = netlink.LinkSetMTU(bridge, mtu)
	...
	if err := netlink.LinkSetMTU(vx, mtu); err != nil {
		return err
	}
	...
	for _, route := range routes {
		if route.Protocol == gliderRouteProtocol {
			_ = netlink.RouteDel(&route)
		}
	}
	neighbors, _ := netlink.NeighList(vx.Attrs().Index, netlink.FAMILY_V4)
	...
	for _, peer := range peers {
		if peer.NodeID == "" || !peer.PodCIDR.IsValid() || !peer.PodCIDR.Addr().Is4() || !peer.TunnelAddress.Is4() {
			return fmt.Errorf("invalid overlay peer %q", peer.NodeID)
		}
		if seen[peer.PodCIDR.String()] {
			return fmt.Errorf("duplicate peer subnet %s", peer.PodCIDR)
		}
		seen[peer.PodCIDR.String()] = true
		dst := net.IPNet{IP: net.IP(peer.PodCIDR.Masked().Addr().AsSlice()), Mask: net.CIDRMask(peer.PodCIDR.Bits(), 32)}
		if err := netlink.RouteReplace(&netlink.Route{LinkIndex: bridge.Attrs().Index, Dst: &dst, Protocol: gliderRouteProtocol, Scope: netlink.SCOPE_LINK}); err != nil {
			return err
		}
		...
		if err := netlink.NeighAppend(flood); err != nil && !errors.Is(err, os.ErrExist) {
			return fmt.Errorf("add VXLAN peer %s: %w", peer.NodeID, err)
		}
	}
...
func durableWrite(final string, data []byte) error {
	...
	if d, openErr := os.Open(filepath.Dir(final)); openErr == nil {
		_ = d.Sync()
		_ = d.Close()
	}
	return nil
