...
if [ "$(uname -s)" != Linux ] || ! command -v go >/dev/null 2>&1; then
	exec docker run --rm -v "${REPO_ROOT}:/src" -v glider-go-mod:/go/pkg/mod -v glider-go-build:/root/.cache/go-build -w /src golang:1.26.6 bash scripts/test-packaged-ha.sh
fi
