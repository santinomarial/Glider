...
func NewManager(root string) (*Manager, error) {
	if root == "" || !filepath.IsAbs(root) {
		return nil, fmt.Errorf("%w: snapshot root must be absolute", ErrInvalidSnapshot)
	}
...
func (m *Manager) Ensure(id string, lowerDirs []string) (Snapshot, error) {
	...
	if len(lowerDirs) == 0 {
		return Snapshot{}, fmt.Errorf("%w: no lower layers", ErrInvalidSnapshot)
	}
	clean := make([]string, len(lowerDirs))
	for i, dir := range lowerDirs {
		if !filepath.IsAbs(dir) || strings.ContainsAny(dir, ",:") {
			return Snapshot{}, fmt.Errorf("%w: unsafe lowerdir %q", ErrInvalidSnapshot, dir)
		}
		info, err := os.Stat(dir)
		if err != nil || !info.IsDir() {
			return Snapshot{}, fmt.Errorf("%w: lowerdir %q is not a directory", ErrInvalidSnapshot, dir)
		}
	...
	if err := sameFilesystem(s.Upper, s.Work); err != nil {
		return Snapshot{}, err
	}
	if err := saveRecord(filepath.Dir(s.Upper), Record{Version: 1, ID: id, LowerDirs: clean}); err != nil {
		return Snapshot{}, err
	}
	reversed := make([]string, len(clean))
	for i := range clean {
		reversed[len(clean)-1-i] = clean[i]
	}
	data := "lowerdir=" + strings.Join(reversed, ":") + ",upperdir=" + s.Upper + ",workdir=" + s.Work
	if err := syscall.Mount("overlay", s.Merged, "overlay", 0, data); err != nil {
		return Snapshot{}, fmt.Errorf("mount overlay snapshot %s: %w", id, err)
	}
	return s, nil
}
...
func (m *Manager) Remove(id string) error {
	...
	if mounted {
		if err := syscall.Unmount(s.Merged, syscall.MNT_DETACH); err != nil {
			return fmt.Errorf("unmount snapshot %s: %w", id, err)
		}
	}
	if err := os.RemoveAll(filepath.Dir(s.Upper)); err != nil {
...
func (m *Manager) paths(id string) (Snapshot, error) {
	if !idRE.MatchString(id) {
		return Snapshot{}, fmt.Errorf("%w: invalid id %q", ErrInvalidSnapshot, id)
	}
...
func sameFilesystem(a, b string) error {
	...
	if sa.Dev != sb.Dev {
		return fmt.Errorf("%w: upper and work directories are on different filesystems", ErrInvalidSnapshot)
	}
	return nil
}
...
func loadRecord(dir string) (Record, error) {
	...
	if err = json.Unmarshal(data, &rec); err != nil {
		return rec, fmt.Errorf("%w: decode snapshot state: %v", ErrInvalidSnapshot, err)
	}
	return rec, nil
}
