...
type fakeStore struct { services map[string]api.Service; tasks []api.Task }
func (f *fakeStore) ListServices(context.Context) ([]api.Service,error) { var out []api.Service; for _,v := range f.services { out=append(out,v) }; return out,nil }
func (f *fakeStore) ListTasks(context.Context) ([]api.Task,error) { return f.tasks,nil }
func (f *fakeStore) PutService(_ context.Context, s api.Service, _ int64) (api.Service,error) { f.services[s.Metadata.ID]=s; return s,nil }
func TestReconcilePublishesOnlyReadyMatchingValidEndpoints(t *testing.T) {
	...
	c,_:=New(f); if err:=c.Reconcile(context.Background(),svc); err!=nil { t.Fatal(err) }
	got:=f.services["api"].Status.Endpoints
	if len(got)!=1 || got[0].TaskID!="ready" || got[0].Port!=8080 { t.Fatalf("endpoints=%+v",got) }
}
