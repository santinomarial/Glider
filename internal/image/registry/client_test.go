...
func TestFetchManifestBearerChallenge(t *testing.T) {
	...
	server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/token":
			if user, pass, ok := r.BasicAuth(); !ok || user != "u" || pass != "p" { t.Errorf("token request credentials = %q/%q/%v", user, pass, ok) }
			if r.URL.Query().Get("service") != "test" || r.URL.Query().Get("scope") != "repository:team/app:pull" { t.Errorf("token query = %s", r.URL.RawQuery) }
	...
	if err != nil { t.Fatal(err) }
	if string(data) != string(manifest) || desc.Digest != wantDigest { t.Fatalf("unexpected result: %s %s", data, desc.Digest) }
}
...
func TestFetchManifestRejectsAdvertisedDigestMismatch(t *testing.T) {
	...
	if err == nil { t.Fatal("expected digest mismatch") }
}
...
func TestParseChallengeHandlesQuotedComma(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if scheme != "Bearer" || values["realm"] != "https://auth.example/token?a=b,c=d" { t.Fatalf("unexpected parse: %q %#v", scheme, values) }
}
