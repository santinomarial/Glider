...
| Upgrades and schema migration | P0 | Compatibility bounds, mutexed crash-resumable migration, guarded rollback, concurrency tests, packaged mixed v1/v2 API mutation qualification, and current/legacy TLS-etcd canary qualification exist | Release-environment rolling replica exercise |
...
| Performance qualification | P1 | Published baseline sizing, saturation signals, p99 operational SLO, mean/allocation guards, exact-commit throughput plus p50/p95/p99 reports for simulated control-plane workloads, real-kernel warm-rootfs lifecycles, and cold local-registry verified pull/unpack/run lifecycles exist | Release-registry and release-hardware multi-host load/saturation qualification |
