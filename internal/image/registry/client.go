...
func (c *Client) FetchManifest(ctx context.Context, ref reference.Reference, selector string) ([]byte, v1.Descriptor, error) {
	...
	if err != nil {
		return nil, v1.Descriptor{}, fmt.Errorf("read manifest: %w", err)
	}
	if len(data) > maxManifestBytes {
		return nil, v1.Descriptor{}, fmt.Errorf("%w: manifest exceeds %d bytes", ErrProtocol, maxManifestBytes)
	}
	...
	if advertised := resp.Header.Get("Docker-Content-Digest"); advertised != "" {
		...
		if err != nil || want.Validate() != nil {
			return nil, v1.Descriptor{}, fmt.Errorf("%w: invalid Docker-Content-Digest %q", ErrProtocol, advertised)
		}
		if want != desc.Digest {
			return nil, v1.Descriptor{}, fmt.Errorf("%w: manifest response hashes to %s, registry advertised %s", content.ErrDigestMismatch, desc.Digest, want)
		}
	}
...
func (c *Client) do(ctx context.Context, registryHost, path string, headers http.Header) (*http.Response, error) {
	...
	if resp.StatusCode == http.StatusUnauthorized {
		...
		resp, err = c.http.Do(req)
		if err != nil {
			return nil, fmt.Errorf("authenticated registry GET %s: %w", path, err)
		}
	}
...
func (c *Client) bearerToken(ctx context.Context, registryHost, challenge string) (string, error) {
	...
	if err != nil || !strings.EqualFold(scheme, "Bearer") {
		return "", fmt.Errorf("%w: unsupported WWW-Authenticate challenge %q", ErrUnauthorized, challenge)
	}
	...
	if c.credentials != nil {
		if cred, ok := c.credentials(registryHost); ok {
	...
	defer resp.Body.Close()
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return "", fmt.Errorf("%w: token service returned %s", ErrUnauthorized, resp.Status)
	}
	var payload struct {
	...
	if err := json.NewDecoder(io.LimitReader(resp.Body, 1<<20)).Decode(&payload); err != nil {
		return "", fmt.Errorf("%w: decode token response: %v", ErrProtocol, err)
	}
	if payload.Token == "" {
		payload.Token = payload.AccessToken
	}
	if payload.Token == "" {
		return "", fmt.Errorf("%w: token response contained no token", ErrProtocol)
	}
...
func parseChallenge(value string) (string, map[string]string, error) {
	...
	if space <= 0 {
		return "", nil, errors.New("missing authentication parameters")
	}
	...
	for len(strings.TrimSpace(rest)) > 0 {
		...
		if eq <= 0 {
			return "", nil, errors.New("malformed authentication parameter")
		}
		key := strings.ToLower(strings.TrimSpace(rest[:eq]))
		rest = strings.TrimSpace(rest[eq+1:])
		if !strings.HasPrefix(rest, `"`) {
			return "", nil, errors.New("authentication value must be quoted")
		}
		rest = rest[1:]
		...
		if !closed {
			return "", nil, errors.New("unterminated authentication value")
		}
		...
		if rest[0] != ',' {
			return "", nil, errors.New("malformed authentication parameter separator")
		}
		rest = rest[1:]
	}
