...
func TestHealthDaemonReportsReadinessAndDefersRestart(t *testing.T) {
	...
	store := &healthFakeStore{assignments: []api.Assignment{a}}
	daemon := NewHealthDaemon("node", store, healthFakeChecker{}, time.Millisecond)
	...
	daemon.checker = healthFakeChecker{fail: true}
	key := containerID(a)
	daemon.next[key] = time.Time{}
	daemon.reconcile(context.Background(), store.assignments)
	if store.restarts != 0 || !daemon.pending[key] {
		t.Fatal("restart was not deferred by backoff")
	}
	daemon.next[key] = time.Now().Add(-time.Second)
	daemon.reconcile(context.Background(), store.assignments)
	if store.restarts != 1 {
